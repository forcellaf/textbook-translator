...
2. The historical diagnostics must still *detect* the defects the current
   parser no longer produces (`$x $` padding, `$$$$` welds, unbalanced
   braces, over-tabbed arrays). Those fixtures are the guard against a future
   parser regression slipping through unnoticed, which is the whole reason
   the checks were kept after the normalizer passes were dropped.
"""
...
def test_delimiter_whitespace_is_detected() -> None:
    report = lint_markdown(r"A span $n S { \mathrm { d } } l $ that pandoc will not read.")
...
def test_a_flat_outline_is_flagged() -> None:
    """The real defect: 422 of 442 headings at level 2, so pandoc renders
    chapters, sections and examples at identical weight."""
...
def test_expansion_outlier_catches_a_truncation_the_floor_misses(tmp_path: Path) -> None:
    ...
    for index in range(1, 6):
        name = f"{index:03d}.md"
        (kit / "chunks" / name).write_text(UNTRANSLATED_CHUNK, encoding="utf-8")
...
def test_expansion_outlier_stays_quiet_on_a_consistent_book(tmp_path: Path) -> None:
    ...
    for index in range(1, 6):
        name = f"{index:03d}.md"
        (kit / "chunks" / name).write_text(UNTRANSLATED_CHUNK, encoding="utf-8")
        (kit / "translated" / name).write_text(TRANSLATED_CHUNK, encoding="utf-8")
...
def test_lint_kit_exits_zero_when_clean(tmp_path: Path) -> None:
    kit = _make_kit(tmp_path, {"001.md": TRANSLATED_CHUNK})
    report = lint_kit(kit)
...
def test_lint_kit_exits_non_zero_when_review_is_needed(tmp_path: Path) -> None:
    kit = _make_kit(tmp_path, {"001.md": UNTRANSLATED_CHUNK})
    report = lint_kit(kit)
...
def test_lint_kit_strips_a_wrapping_code_fence(tmp_path: Path) -> None:
    kit = _make_kit(tmp_path, {"001.md": f"```markdown\n{TRANSLATED_CHUNK}\n```"})
    report = lint_kit(kit)
...
def test_lint_kit_resolves_image_tokens_before_checking_paths(tmp_path: Path) -> None:
    ...
    kit = _make_kit(tmp_path, {"001.md": f"{TRANSLATED_CHUNK}\n\n![figure](IMG_0000)\n"})
